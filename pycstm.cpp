...
public:
    ...
    double *get_stylistic_vector(id word_id) {
        double *original_vec = _cstm->get_stylistic_vector(word_id);
        std::memcpy(_vec_copy, original_vec, _cstm->_ndim_d * sizeof(double));
        return _vec_copy;
    }
    ...
    double *get_doc_vector_in_stylistic_space(int doc_id) {
        double *original_vec = _cstm->get_doc_vector_in_stylistic_space(doc_id);
        std::memcpy(_vec_copy, original_vec, _cstm->_ndim_d * sizeof(double));
        return _vec_copy;
    }
    ...
    python::list get_stylistic_vector_by_word(wstring str) {
        id word_id = _vocab->get_word_id(str);
    ...
    python::list get_stylistic_vector_by_id(id word_id) {
    ...
    python::list get_stylistic_vectors() {
        python::list vector_array;
        for (id word_id=0; word_id<get_vocabulary_size(); ++word_id) {
            double *vector = get_stylistic_vector(word_id);
            vector_array.append(_convert_vector_to_list(vector));
    ...
    python::list get_doc_vectors_in_semantic_space() {
        ...
        for (int doc_id=0; doc_id<get_num_documents(); ++doc_id) {
            python::list vector_list;
            double *vector = get_doc_vector_in_semantic_space(doc_id);
            for (int i=0; i<_cstm->_ndim_d; ++i) {
                vector_list.append(vector[i]);
            }
    ...
    python::list get_doc_vectors_in_stylistic_space() {
        python::list vector_array;
        for (int doc_id=0; doc_id<get_num_documents(); ++doc_id) {
            python::list vector_list;
            double *vector = get_doc_vector_in_stylistic_space(doc_id);
            for (int i=0; i<_cstm->_ndim_d; ++i) {
                vector_list.append(vector[i]);
            }
            vector_array.append(vector_list);
    ...
    python::list get_high_freq_words(size_t size=100) {
        ...
        for (int n=0; n<std::min(size, ranking.size()); ++n) {
            ...
            tuple.append(word);
            tuple.append(count);
            tuple.append(_convert_vector_to_list(semantic_vector));
            tuple.append(_convert_vector_to_list(stylistic_vector));
            result.append(tuple);
            itr++;
        }
        return result;
    ...
    python::list get_docs_similar_to_file_in_semantic_space(string filename, size_t size=10) {
        ...
        std::memcpy(target_vec, get_doc_vector_in_semantic_space(doc_id), ndim_d * sizeof(double));
        python::list result;
        _get_docs_similar_to_raw_vector_in_semantic_space(target_vec, size, result);
        delete[] target_vec;
        return result;
    }
    python::list get_docs_similar_to_vector_in_semantic_space(python::list vector_list, size_t size=10) {
        int ndim_d = _cstm->_ndim_d;
    ...
    void _get_docs_similar_to_raw_vector_in_semantic_space(double *target_vec, size_t size, python::list &result) {
        int ndim_d = _cstm->_ndim_d;
        std::pair<int, double> pair;
        multiset<std::pair<int, double>, multiset_comparator<double>> ranking;
        for(int doc_id=0; doc_id<get_num_documents(); ++doc_id) {
            double *vec = get_doc_vector_in_semantic_space(doc_id);
    ...
    python::list get_docs_similar_to_file_in_stylistic_space(string filename, size_t size=10) {
        int doc_id = get_doc_id_by_filename(filename);
        int ndim_d = _cstm->_ndim_d;
        double *target_vec = new double[ndim_d];
        std::memcpy(target_vec, get_doc_vector_in_stylistic_space(doc_id), ndim_d * sizeof(double));
        python::list result;
        _get_docs_similar_to_raw_vector_in_stylistic_space(target_vec, size, result);
        delete[] target_vec;
        return result;
    }
    python::list get_docs_similar_to_vector_in_stylistic_space(python::list vector_list, size_t size=10) {
        int ndim_d = _cstm->_ndim_d;
        assert(python::len(vector_list) == ndim_d);
        double *target_vec = new double[ndim_d];
        for(int i=0; i<ndim_d; ++i) {
    ...
    void _get_docs_similar_to_raw_vector_in_stylistic_space(double *target_vec, size_t size, python::list &result) {
        int ndim_d = _cstm->_ndim_d;
        std::pair<int, double> pair;
        multiset<std::pair<int, double>, multiset_comparator<double>> ranking;
        for(int doc_id=0; doc_id<get_num_documents(); ++doc_id) {
            double *vec = get_doc_vector_in_stylistic_space(doc_id);
            double f = cstm::inner(vec, target_vec, ndim_d) / (cstm::norm(vec, ndim_d) * cstm::norm(target_vec, ndim_d));
            pair.first = doc_id;
            pair.second = f;
            ranking.insert(pair);
...
BOOST_PYTHON_MODULE(pycstm) {
    ...
    .def("get_semantic_vector_by_word", &PyCSTM::get_semantic_vector_by_word)
    .def("get_stylistic_vector_by_word", &PyCSTM::get_stylistic_vector_by_word)
    .def("get_semantic_vectors", &PyCSTM::get_semantic_vectors)
    ...
    .def("get_docs_similar_to_vector_in_stylistic_space", &PyCSTM::get_docs_similar_to_vector_in_stylistic_space)
